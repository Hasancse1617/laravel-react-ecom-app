                  ...
                  <div class="card-body">
                      ...
                      <tbody>
                    {
                      ...
                      categories.map((category, index)=>(
                          ...
                          <td>{ category.url }</td>
                          <td><img width="100" width="100" src={`${process.env.REACT_APP_API_PATH}/images/category_images/${category.category_image}`}/></td>
                          <td>
                              {
                                (category.status === 1) ? 
                                <a class="updateCategoryStatus" data-category={category.id} id={`category-${category.id}`} onClick={categoryStatus} href="javascript:void(0)"> <i class="fas fa-toggle-on" status={category.status} aria-hidden="true"></i></a>
                                :<a class="updateCategoryStatus" data-category={category.id} id={`category-${category.id}`} onClick={categoryStatus} href="javascript:void(0)"> <i class="fas fa-toggle-off" status={category.status} aria-hidden="true"></i> </a> 
                              }
                          </td>
                          <td>
                            <NavLink exact to={`/admin/category/edit/${category.id}`} ><button className="text-success" ><i className="fas fa-edit"></i></button></NavLink>&nbsp;
                            <button onClick={() => deleteCategory(category.id)} className="text-danger"><i className="fas fa-trash"></i></button>&nbsp;
                          </td>
                        </tr>
                        ))
                        :'No Categories found'
