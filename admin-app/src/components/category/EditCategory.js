...
import { updateAction, parentcategories, fetchCategory } from "../../store/actions/CategoryAction";
...
const EditCategory = (props) => {
    ...
    const { id } = useParams();
    const { loading, categoryErrors, redirect, parent_categories, category, status } = useSelector((state)=> state.CategoryReducer);
    ...
    const updateCategory = (e) =>{
        e.preventDefault();
        const {category_name,parent_id,category_image} = state;
    ...
    useEffect(()=>{
        ...
        if(Object.keys(categoryErrors).length > 0){
            for(const error in categoryErrors){
                toast.error(categoryErrors[error])
            }
        ...
        <div class="container-fluid">
            ...
            <div class="col-12">
                ...
                {!loading?<form role="form" onSubmit={updateCategory}>
                    ...
                    <div class="form-group row">
                        <label for="exampleInput" className="col-sm-2  col-form-label">Parent Category</label>
                        <div className="col-sm-8">
                          <select class="form-control" name="parent_id" onChange={handleInput}>
                              <option value="">Select Parent Category</option>
                              {
                                  parent_categories.map((cat)=>(
                                    <option value={ cat.id } selected={category.parentcategory? cat.id===category.parentcategory.id : ''} >{ cat.category_name }</option>
                                  ))
                              }
                          </select>
                        </div> 
